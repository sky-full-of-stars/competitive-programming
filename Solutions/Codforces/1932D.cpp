...
void solve()
{
	...
	for (int i = 0; i < sz(cards); i++)
	{
		for (int j = 0; j < sz(cards); j++)
		{
			...
			{
				string minCard = min(cards[i], cards[j]);
				string mxCard = max(cards[i], cards[j]);
				ans.pb({minCard, mxCard});
				cards[i] = "-";
				cards[j] = "-";
			}
		}
	...
	else
	{
		...
		{
			sortv(tCards);
			for (int i = 0; i < sz(tCards); i++)
			{
				ans.pb({tCards[i], tCards.back()});
				tCards.pop_back();
			}
		}
	}
