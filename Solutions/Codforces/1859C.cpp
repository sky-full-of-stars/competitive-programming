...
bool isSumPossible(const std::vector<std::vector<int>>& matrix, int targetSum) {
	int n = matrix.size();
...
bool possible(int x, const std::vector<std::vector<int>>& matrix)
{
	vi sub = matrix[sz(matrix) - 1];
	debug(sub);
	for (auto i : sub)
	{
		if (x - i > 0 and isSumPossible(matrix, x - i))
		{
			return true;
		}
	}
