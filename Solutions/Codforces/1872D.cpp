...
void solve()
{
	...
	int sumTerms = n / a;
	int difTerms = n / b;
	int commonTerms = n / lcm(a, b);
	sumTerms -= commonTerms;
	difTerms -= commonTerms;

	int ans = 0;
	int addAllSum = sigma(n) - sigma(n - sumTerms);
	int difAllSum = sigma(difTerms);
	ans = addAllSum - difAllSum;
	cout << ans << endl;
	return;
